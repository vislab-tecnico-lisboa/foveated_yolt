...
cv::Mat foveate(const cv::Mat &img, 
		const int &size_map,
		const int &levels, 
		const int &sigma_x,
		const int &sigma_y,
...
int main(int argc, char** argv)
{
	...
	string foveation_filename   ="/home/rui/ws/src/foveated_yolt/images/foveation_";
	string saliency_map_filename="/home/rui/ws/src/foveated_yolt/images/saliencymap_";
	string bbox_filename        ="/home/rui/ws/src/foveated_yolt/images/bbox_";
		...
		for (unsigned int sigma_index = 0;sigma_index < sigmas.size(); ++sigma_index)
		{
			...
			for (unsigned int fixedpt_index = 0; fixedpt_index < fixedpts.size(); ++fixedpt_index)
			{
				...
				for (unsigned int input=0; input<total_images; ++input)
				{
					...
					img_orig.convertTo(img_orig, CV_64F);
					...
					if(mode==FOVEATION)
					{
						fixedpt.at<int>(0,0)=img.rows/2.0;
						fixedpt.at<int>(1,0)=img.cols/2.0;

						img=foveate(img_orig,size_map,levels,sigma,sigma,fixedpt);
					}
					else
					{
						cv::GaussianBlur(img_orig,img,Size(5,5),sigma,sigma);
					}
					
					cv::Mat img_first_pass=img.clone();

					// Uncomment for Visualize Foveated Image
					//Network.VisualizeFoveation(fixedpt,img_first_pass,sigma,fixedpt_index,foveation_filename);
					...
					for (int class_index = 0; class_index < N; ++class_index) 
					{
						...
						cv::Mat saliency_map;
						img_orig.convertTo(img_orig, CV_64F);
						//std::cout << img_orig << std::endl;
						cv::Rect Min_Rect = Network.CalcBBox(class_index,img_orig,first_pass_data,thresh,saliency_map);//,fixedpt);
						...
						//exit(-1);
						// 2nd Feedforward with foveated image - Prediciton New top 5 of each predicted class
						...
						if(debug) 
						{
							...
							// CV_64F
							img_orig.convertTo(img_orig, CV_8U);
							img_first_pass.convertTo(img_first_pass, CV_8U);
							img_second_pass.convertTo(img_second_pass, CV_8U);
