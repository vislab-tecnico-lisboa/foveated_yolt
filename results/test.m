...
sigmas_leg = [1 20 40 60 80 100];
plot_google_feedback_detection = [detection_foveate_google_error_rate(1,:); detection_foveate_google_error_rate(3,:); detection_foveate_google_error_rate(5,:); detection_foveate_google_error_rate(7,:); detection_foveate_google_error_rate(9,:); detection_foveate_google_error_rate(11,:)];
legend_sigma = {};
for i=1:length(sigmas_leg)
    legend_sigma = [legend_sigma, strcat('\sigma=', num2str(sigmas_leg(i))) ];
end
...
fontsize=30;
set(gcf, 'Color', [1,1,1]);  % 
plot(threshs,100*detection_google_error_rate(9,:), 'k-s');   % 1 passagem
hold on
