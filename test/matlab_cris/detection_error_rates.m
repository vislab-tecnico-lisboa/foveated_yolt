function [l_error_pos,error_av,error_std] = detection_error_rates(sigmas,threshs,fix_pts,images_number,detections,gt_detections,detections_resolution,top_k,overlap_correct)
...
l_error_pos=((size(max_overlap,4))*(size(max_overlap,2))*(size(max_overlap,1))-...
    sum(sum(sum(max_overlap,4),2),1))/((size(max_overlap,4))*(size(max_overlap,2))*(size(max_overlap,1)));
